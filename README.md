...
**Loaded on intent, not with the page (v2.7.1+).** Turnstile's `api.js` is injected only on the first tap/focus inside an opt-in form, and the widget renders into that form (an invisible-mode widget shows nothing; a visible one grows the form while the visitor types). Loading it with the page (2.7.0) ran the challenge on every visit, and on Android — where Chrome keeps cross-origin iframes on the page's main thread — that delayed taps anywhere on the page (Search Console INP issue on eco, Sep 2026). Submit waits up to 20 s for the token before showing the error message.
